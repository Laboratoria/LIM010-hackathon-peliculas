...
const app = {
	...
	getMoviesByGenres: async (tmdbApiKey,genreId)=> {
		const movies = await fetch(`https://api.themoviedb.org/3/discover/movie?&apikey=${tmdbApiKey}&language=es-ES&sort_by=popularity.desc&certification_country=US&certification=G&include_adult=false&include_video=false&with_genres=${genreId}&with_original_language=en`);
		const movieResolved = await movies.json();	
		return movieResolved;
	},
